...
function read_quotes_file()
  ...
  if f == nil then
    print ('Created a new facts file on '..quotes_file)
    serialize_to_file({}, quotes_file)
  else
    print ('Facts loaded: '..quotes_file)
    f:close()
  end
...
function save_quote(msg)
  ...
  if quotes_table[to_id] == nil then
    print ('New fact key to_id: '..to_id)
    quotes_table[to_id] = {}
  end
...
function run(msg, matches)
  if string.match(msg.text, "!fact$") then
    return get_quote(msg)
  elseif string.match(msg.text, "!addfact (.+)$") then
    quotes_table = read_quotes_file()
...
return {
  ...
  description = "fact plugin, you can create and retrieve random facts",
  usage = {
    "!addfact [msg]",
    "!fact",
  },
